...
function populate_table_main(){ 
	...
	$.ajax ({
	  type: "POST",
	  url: "../../model/order/populate_table_main.php",
	  dataType: 'json',      
	  cache: false,
	  success: function(s)
	  {
...
function populate_products_dropdown(){ 
  //ajax now
  $.ajax ({
    type: "POST",
    ...
    {
      $('#f_products').empty();
      $('#f_products').html('<option selected="selected" value="none">--SEARCH PRODUCTS--</option>');
      for(var i = 0; i < s.length; i++) { 
        $('#f_products').append('<option id="opt'+s[i][0]+'" value="'+s[i][0]+'">'+s[i][1]+'</option>');
      }       
    }  
  }); 
  //ajax end  
...
function showTextBox(get){
	$('#f_servicelabel_div').removeClass('has-error');     
	$('#f_category_div').removeClass('has-error'); 	
	$('#f_service_div').removeClass('has-error'); 	
	$('#f_productlabel_div').removeClass('has-error'); 	
	$('#f_products_div').removeClass('has-error'); 
	$('#f_qty_div').removeClass('has-error'); 
	$('#f_total_div').removeClass('has-error'); 			
	if(get=='services'){
