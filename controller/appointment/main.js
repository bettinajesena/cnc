...
function populate_table_main(){ 
	...
	$.ajax ({
	  type: "POST",
	  url: "../../model/appointment/populate_table_main.php",
	  dataType: 'json',      
	  cache: false,
	  success: function(s)
	  {
		
		populate_service_dropdown();
...
var client_id = client_id; 
 $('#f_pet').empty();
  ...
  $.ajax ({
    ...
    {
      $('#f_pet').empty();
      $('#f_pet').html('<option selected="selected" value="none">--SEARCH PET--</option>');
      for(var i = 0; i < s.length; i++) { 
        $('#f_pet').append('<option id="opt'+s[i][1]+'" value="'+s[i][1]+'">'+s[i][2]+'</option>');
      }       
    }  
  });
  //ajax end
...
function populate_service_dropdown(){ 
  //ajax now
  $.ajax ({
    type: "POST",
    ...
    {
      $('#f_service').empty();
      $('#f_service').html('<option selected="selected" value="none">--SEARCH SERVICE--</option>');
      for(var i = 0; i < s.length; i++) { 
        $('#f_service').append('<option id="opt'+s[i][0]+'" value="'+s[i][0]+'">'+s[i][1]+'</option>');
      }       
    }  
  }); 
  //ajax end  
