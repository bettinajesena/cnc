...
// Cost Related Variables
	// The currency tag in HTML such as &euro; &pound; &yen;
...
function make_booking_array($year, $month, $j = 0) { 
	...
	$stmt = $this->link->prepare("SELECT name, date, start FROM appointment WHERE date LIKE  CONCAT(?, '-', ?, '%')"); 
...
function make_cells($table = '') {
	...
	foreach($this->days as $i => $r) { // Loop through the date array
		...
		if(isset($this->bookings_per_day[$current_day]) && $tag == 0) {
			...
			$current_day_slots_booked = count($this->bookings_per_day[$current_day]);
				...
				if($current_day_slots_booked < $this->slots_per_day) {
					...
					<a href='main.php?month=" .  $this->month . "&amp;year=" .  $this->year . "&amp;day=" . sprintf("%02s", $r['daynumber']) . "' class='part_booked' title='This day is part booked'>" . 
					$r['daynumber'] . "</a></td>"; 
					$tag = 1;
					...
					echo "\r\n<td width='21' valign='top'>
					<a href='main.php?month=" .  $this->month . "&amp;year=" .  $this->year . "&amp;day=" . sprintf("%02s", $r['daynumber']) . "' class='fully_booked' title='This day is fully booked'>" . 
					$r['daynumber'] . "</a></td>"; 
					$tag = 1;			
		...
		if($tag == 0) {
			...
			echo "\r\n<td width='21' valign='top'>
			<a href='main.php?month=" .  $this->month . "&amp;year=" .  $this->year . "&amp;day=" . sprintf("%02s", $r['daynumber']) . "' class='green' title='Please click to view bookings'>" . 
			$r['daynumber'] . "</a></td>";			
...
function booking_form() {
	...
	<table width='220' border='0' cellpadding='2' cellspacing='0' id='booking'>
		<tr>
			<th width='100' align='left'>Start</th>
			<th width='100' align='left'>End</th>
...
echo"<label>Client Name <span class='glyphicon glyphicon-asterisk' aria-hidden='true' style='color:orange'></span></label> <!-- Prod_Name -->
    <select type='text' class='form-control' name='f_owner' id='f_owner' required onchange='getPet(this.value);'>
    	<option value='none'> -- SEARCH Client Name -- </option>";

    echo"
    </select>
   
<label>Pet Name <span class='glyphicon glyphicon-asterisk' aria-hidden='true' style='color:orange'></span></label> <!-- Prod_Name -->
    <select type='text' class='form-control' name='f_pet' id='f_pet' required>
    	<option value='none'> -- SEARCH Pet Name -- </option>";

    echo"
    </select>
			
			<label>Service to Avail <span class='glyphicon glyphicon-asterisk' aria-hidden='true' style='color:orange'></span></label> <!-- Prod_Name -->
    <select type='text' class='form-control' name='f_service' id='f_service' required>
    	<option value='none'> -- SEARCH Service -- </option>";


    	$query = $db->query("SELECT * FROM services where status='active'");
...
$rowCount = $query->num_rows;
    ...
    <label>Details</label> 
    <textarea rows='3' style='resize:none' class='form-control' name='details' id='details'></textarea>
                    ...
                    <div><br><button id='btn_add' class='btn btn-block btn-success btn-lg'>SAVE</button></div>
